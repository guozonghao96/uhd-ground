...
class ModelArguments:
    ...
    # 
    mm_use_loc_token: bool = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # 
    tune_word_embedding: bool = field(default=False)
    pretrain_word_embedding: bool = field(default=False)
    tune_vision_enc: bool = field(default=False)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        if getattr(trainer.args, "tune_word_embedding", False):
...
class LazySupervisedDataset(Dataset):
    ...
    def resize_as_grid(self, image, slice_num):
        w, h = image.size
        r = math.sqrt(slice_num * 336 * 336 / h / w) # 保证是切为slice_num片
        fix_h = int(h * r)
        fix_w = int(w * r)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0]:
                ...
                if 'textvqa' in image_file: 
                    ...
                    if random.random() < 0.1 and len(temp_patches) != 6:
                        image = self.resize_as_grid(image, slice_num=6)
            ...
            if 'image' in self.list_data_dict[i]:
                ...
                data_dict['patch_images'] = patch_images
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.initialize_vision_tokenizer(model_args, training_args, tokenizer=tokenizer)
    ...
    if training_args.tune_vision_enc:
        model.get_model().vision_tower.requires_grad_(True)
        # model.get_model().vision_tower.vision_tower.gradient_checkpointing = False
        # model.get_model().vision_tower.gradient_checkpointing = False
        # 这些都不管用
