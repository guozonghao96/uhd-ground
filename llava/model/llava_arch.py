...
class LlavaMetaForCausalLM(ABC):
    ...
    def initialize_vision_tokenizer(self, model_args, training_args, tokenizer):
        ...
        # add for training word embeddings
        elif model_args.mm_use_loc_token:
            special_tokens_dict = {'additional_special_tokens': ['<ph>', '</ph>', '<box>', '</box>']}
            num_new_tokens = tokenizer.add_special_tokens(special_tokens_dict)
            self.resize_token_embeddings(len(tokenizer))
            # initialize
            if num_new_tokens > 0:
                input_embeddings = self.get_input_embeddings().weight.data # word_embeding
                output_embeddings = self.get_output_embeddings().weight.data # lm_head
                input_embeddings_avg = input_embeddings[:-num_new_tokens].mean(
                    dim=0, keepdim=True)
                output_embeddings_avg = output_embeddings[:-num_new_tokens].mean(
                    dim=0, keepdim=True)
            ...
            if training_args.pretrain_word_embedding:
                mm_projector_weights = torch.load(model_args.pretrain_mm_mlp_adapter, map_location='cpu')
                embed_tokens_weight = mm_projector_weights['model.embed_tokens.weight']
                assert num_new_tokens == 4, "spetial token must be '<ph>', '</ph>', '<box>', '</box>'"
                if input_embeddings.shape == embed_tokens_weight.shape:
                    # input_embeddings[-num_new_tokens:] = embed_tokens_weight[-num_new_tokens:]
                ...
                if output_embeddings.shape == lm_head_weight.shape:
                    ...
                    output_embeddings[:] = lm_head_weight[:]
                    # output_embeddings = lm_head_weight # 直接这样不能赋值
                else:
                    raise ValueError(f"Unexpected lm_head_weight shape. Pretrained: {lm_head_weight.shape}. Current: {output_embeddings.shape}. Numer of new tokens: {num_new_tokens}.")

            if training_args.tune_word_embedding:
                for p in self.get_input_embeddings().parameters():
                    p.requires_grad = True
                for p in self.get_output_embeddings().parameters():
                    p.requires_grad = True
