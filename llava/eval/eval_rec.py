...
def eval_rec(answers, labels):
    ...
    for answer, annotation in zip(answers, labels):
        ...
        img = Image.open('./playground/data/eval/refcoco/train2014/' + annotation['image'])
        ...
        try:
            ...
            pred = torch.as_tensor([float(xmin) / 100 * width_ori, float(ymin) / 100 * height_ori, 
                                    float(xmax) / 100 * width_ori, float(ymax) / 100 * height_ori])
